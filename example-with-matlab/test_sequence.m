...
% throttle testing -------
%throttleValuesToTest = 600:50:1800;
%throttleValuesToTest = flip(throttleValuesToTest);   % test high throttle values first for motor overheating reasons
%rotorResponseValues = cell2mat(cellfun(@(throttleValue) evaluateRotor(hingeName, 0, throttleValue, unitTestlength, measurementInstrumentation, readingRate, rampTime, lowPassFilterFrequency), num2cell(throttleValuesToTest), 'UniformOutput', false));
...
%figure;
%plot(throttleValuesToTest, rotorResponseValues(3,:), '.r', 'MarkerSize',20);
%figure;
%plot(rotorResponseValues(2,:), rotorResponseValues(3,:), '.r', 'MarkerSize',20);